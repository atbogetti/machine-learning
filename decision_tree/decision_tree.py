...
#regr.fit(pca_train, y_train, W_train)
...
for i, num in enumerate(importances):
    if num == 0:
        continue
    if num > 0.01:
        plt.bar(X_names[i], num, color='tomato')
    elif 0.01 > num > 0.05:
        plt.bar(X_names[i], num, color='dodgerblue')
...
#y_pred = regr.predict(pca_test)

y_probs = regr.predict_proba(X_test)
y_probs = y_probs[:,1]
